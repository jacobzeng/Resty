...
public class User extends Model<User> {
    ...
    public final static String FIELD_DELETED_AT = "deleted_at";
    public final static String FIELD_CREATED_AT = "created_at";

    private String origPassword;

    public static void cleanCredentialsCache(String username) {
        Subject.removePrincipal(username);
        Subject.removeAllCredentials();
    }
    ...
    public Role getRole() {
        ...
        if (this.get("role") == null && this.get("id") != null) {
            ...
            role = Role.dao.findFirst(sql, this.<Long>get("id"));
            this.put("role", role);
        } else {
            role = this.get("role");
        }
